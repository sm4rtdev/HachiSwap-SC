...
contract PairFees {
    ...
    uint256 public toPass0;
    uint256 public toPass1;
    ...
    function processPassFees(uint amount, bool isTokenZero) external {
        require(msg.sender == pair);
    ...
    function withdrawPassFees(address recipient) external {
        require(msg.sender == pair);

        if (toPass0 > 0){
            _safeTransfer(token0, recipient, toPass0);
            toPass0 = 0;
        }
        if (toPass1 > 0){
            _safeTransfer(token1, recipient, toPass1);
            toPass1 = 0;
        }
    }
...
contract Pair is IPair {
    ...
    function claimPassFees() external {
        address _feehandler = PairFactory(factory).passFeeHandler();
        PairFees(fees).withdrawPassFees(_feehandler);
    }
    ...
    function _update0(uint amount) internal {
        ...
        // get pass nft fee
        uint256 _passNftFee =  amount * PairFactory(factory).passNFTFee() / 10000;
        PairFees(fees).processPassFees(_passNftFee, true);

        // remove pass nft fees from lpfees
        amount -= _passNftFee;
        ...
        emit Fees(msg.sender, amount+_passNftFee, 0);
    }
    ...
    function _update1(uint amount) internal {
        ...
        // get pass nft fee
        uint256 _passNftFee =  amount * PairFactory(factory).passNFTFee() / 10000;
        PairFees(fees).processPassFees(_passNftFee, false);
        ...
        emit Fees(msg.sender, 0, amount+_passNftFee);
    }
...
contract PairFactory is IPairFactory {
    ...
    uint256 public passNFTFee;
    uint256 public constant MAX_FEE = 50; // 0.5%
    address public feeManager;
    address public pendingFeeManager;
    address public passFeeHandler;   // pass nft fee handler
    ...
    constructor() {
        ...
        stableFee = 1; // 0.01%
        volatileFee = 20;   //0.2%
        passNFTFee = 3000; // 30% of stable/volatileFee
    }
    ...
    function setPassFees(uint256 _newFee) external {
        require(msg.sender == feeManager, 'not fee manager');
        require(_newFee <= 3000);
        passNFTFee = _newFee;
    }

    function setPassFeeAddress(address _feeHandler) external {
        require(msg.sender == feeManager, 'not fee manager');
        require(_feeHandler != address(0));
        passFeeHandler = _feeHandler;
    }
